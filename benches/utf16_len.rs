...
fn bench_inputs(c: &mut Criterion) {
    ...
    for &(name, input) in inputs {
        ...
        group.bench_function(BenchmarkId::new(name, "baseline_64"), |b| {
            b.iter(|| simd_utf16_baseline::utf16_len(black_box(input)));
        });
        group.bench_function(BenchmarkId::new(name, "inline_always_64"), |b| {
            b.iter(|| simd_utf16_inline::utf16_len(black_box(input)));
        });
